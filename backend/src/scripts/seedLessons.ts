...
async function seedLessons() {
  try {
    ...
    console.log(`📚 Found ${courses.length} courses to update with lessons`);
    ...
    for (const course of courses) {
      ...
      if (course.lessons && course.lessons.length > 0) {
        console.log(`⏭️  Skipping "${course.title}" - already has ${course.lessons.length} lessons`);
        skippedCount++;
        continue;
      }
