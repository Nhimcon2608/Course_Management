...
import { lessonApi, CreateLessonData } from '@/services/lessonApi';
...
const CreateLessonPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState<CreateLessonData>({
    ...
    isPublished: false,
    resources: []
    // Note: order will be auto-assigned by backend middleware
  });
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Filter out empty resources and remove order field (let backend handle it)
      const cleanedData = {
        ...formData,
      ...
      // Remove order field to let backend middleware auto-assign it
      delete (cleanedData as any).order;

      await lessonApi.createLesson(courseId, cleanedData);
            ...
            <div className="space-y-8">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex justify-between text-sm">
                      <span className={`${
                        ...
                        {formData.title.length < 3 ? 'Tối thiểu 3 ký tự' : ''}
                      </span>
                      <span className={`${
              ...
              <div>
                ...
                <div>
                  ...
                  />
                  <p className="mt-1 text-sm text-gray-500">
                    Nội dung này sẽ hiển thị cho học viên. Bạn có thể thêm video sau khi tạo bài học.
                  </p>
                </div>
              </div>
